...
function drawPhasor(canvas, form) {
  ...
  // Vẽ I (nét đứt)
  // Khi phi ≈ 0° (I gần trùng U), lệch nhẹ +6° để phân biệt nét đứt vs nét liền
  const OFFSET = 6 * deg; // 6 độ offset khi trùng
  function offsetAngle(angI, angU) {
    const diff = Math.abs(angI - angU) % (2 * Math.PI);
    const minDiff = Math.min(diff, 2 * Math.PI - diff);
    return minDiff < (8 * deg) ? angI + OFFSET : angI; // lệch nếu góc lệch < 8°
  }
  const iA2 = offsetAngle(iA, uA);
  const iB2 = offsetAngle(iB, uB);
  const iC2 = offsetAngle(iC, uC);
  arrow(iA2, parseFloat(Ia)||0, iSc, COL.A, true, 'Ia');
  arrow(iB2, parseFloat(Ib)||0, iSc, COL.B, true, 'Ib');
  arrow(iC2, parseFloat(Ic)||0, iSc, COL.C, true, 'Ic');
      ...
      <div style={{ minHeight:'100vh', background:'#F0F4F8', fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif' }}>
        ...
        <div style={{ maxWidth:520, margin:'0 auto', padding:'14px 12px 40px' }}>
          ...
          {step === 'result' && result && (<>
            ...
            {/* Cảnh báo tải lệch — hiển thị khi thiếu Pa/Pb/Pc mà tải lệch */}
            {(result.warnings||[]).some(w => w.startsWith('UNBALANCED_NO_PABC')) && (() => {
              const w = (result.warnings||[]).find(x => x.startsWith('UNBALANCED_NO_PABC'));
              const ratio = w ? w.split(':')[1] : '';
              return (
                <div style={{
                  background:'#FFF3CD', border:'2px solid #FF8C00',
                  borderRadius:12, padding:'12px 16px', marginBottom:12,
                  display:'flex', gap:12, alignItems:'flex-start',
                }}>
                  <span style={{ fontSize:22, flexShrink:0 }}>⚠️</span>
                  <div>
                    <div style={{ fontSize:13, fontWeight:700, color:'#7D4000', marginBottom:4 }}>
                      Kết quả có thể không chính xác!
                    </div>
                    <div style={{ fontSize:12, color:'#854F0B', lineHeight:1.7 }}>
                      Phụ tải giữa các pha chênh lệch nhau {ratio} lần nhưng chưa có thông số Pa, Pb, Pc từng pha.
                      Tình trạng này dẫn đến tỷ lệ công suất tính toán bị lệch, kết quả chẩn đoán có thể sai.
                    </div>
                    <div style={{ fontSize:12, fontWeight:600, color:'#7D4000', marginTop:6 }}>
                      → Vui lòng nhập đầy đủ Pa, Pb, Pc để tăng độ chính xác.
                    </div>
                  </div>
                </div>
              );
