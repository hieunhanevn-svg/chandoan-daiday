...
// Xây dựng giải thích điểm từng TH bằng ngôn ngữ thực tế
function buildBreakdown(key, ratio, numFlipped, numMissing, has120, numNeg, hasPabc, isHighlyUnbalanced) {
  const lines = [];

  // Giải thích P_ratio
  const ratioMap = {
    'Binh_thuong': { exp: 1.0,    label: 'bằng 100% công suất thực' },
    'TH1':         { exp: 0.667,  label: 'bằng 2/3 công suất thực (mất 1 pha)' },
    'TH2':         { exp: 0.333,  label: 'bằng 1/3 công suất thực (1 CT đảo)' },
    'TH3':         { exp: -0.333, label: 'âm 1/3 công suất thực (2 CT đảo)' },
    'TH4':         { exp: -1.0,   label: 'âm 100% công suất thực (3 CT đảo)' },
    'TH5':         { exp: 0.0,    label: 'gần bằng 0 (đổi chéo 2 pha triệt tiêu)' },
    'TH6':         { exp: 0.667,  label: 'bằng 2/3 công suất thực (2 VT+CT đảo)' },
  };
  const rm = ratioMap[key];
  if (rm) {
    const diff = Math.abs(ratio - rm.exp);
    const match = diff <= 0.20 ? 'KHỚP tốt' : diff <= 0.40 ? 'KHỚP một phần' : 'LỆCH nhiều';
    lines.push(`Tỷ lệ công suất đo/tính = ${(ratio*100).toFixed(0)}%, lý thuyết ${key} phải ${rm.label} — ${match} (lệch ${(diff*100).toFixed(0)}%)`);
  }

  // Giải thích numFlipped
  const flipMap = { 'Binh_thuong':0,'TH1':0,'TH2':1,'TH3':2,'TH4':3,'TH5':-1,'TH6':1 };
  const expFlip = flipMap[key];
  if (expFlip !== -1) {
    const match = numFlipped === expFlip ? 'KHỚP' : `LỆCH (thực tế ${numFlipped}, cần ${expFlip})`;
    if (expFlip === 0) lines.push(`Không phát hiện CT nào đấu ngược — ${match} với yêu cầu của trường hợp này`);
    else lines.push(`Phát hiện ${numFlipped} CT đấu ngược, trường hợp này cần ${expFlip} CT đảo — ${match}`);
  } else {
    lines.push(`Trường hợp TH5 không xét số CT đảo (có thể bất kỳ)`);
  }

  // Giải thích numMissing
  const missMap = { 'Binh_thuong':0,'TH1':1,'TH2':0,'TH3':0,'TH4':0,'TH5':0,'TH6':0 };
  const expMiss = missMap[key];
  if (key === 'TH1') {
    lines.push(numMissing === 1
      ? `Phát hiện đúng 1 pha mất tín hiệu — KHỚP với TH1`
      : `Không thấy pha nào mất tín hiệu hoàn toàn — điểm trừ cho TH1`);
  } else if (numMissing > 0) {
    lines.push(`Có ${numMissing} pha mất tín hiệu trong khi trường hợp này không có — điểm trừ`);
  }

  // Giải thích has120 (TH5 đặc trưng)
  if (key === 'TH5') {
    lines.push(has120
      ? `Phát hiện góc lệch ~120° bất thường giữa các pha — ĐÂY LÀ DẤU HIỆU ĐẶC TRƯNG của TH5, điểm cộng lớn`
      : `Không có góc lệch 120° bất thường — điểm trừ nặng cho TH5, khả năng thấp`);
  } else if (has120) {
    lines.push(`Phát hiện góc lệch 120° bất thường — điểm trừ vì trường hợp này không có đặc trưng đó`);
  }

  // Giải thích ratioX khi có Pa/Pb/Pc
  if (hasPabc && numNeg !== undefined) {
    const negMap = { 'Binh_thuong':0,'TH1':0,'TH2':1,'TH3':2,'TH4':3,'TH5':-1,'TH6':2 };
    const expNeg = negMap[key];
    if (expNeg !== -1) {
      const match = numNeg === expNeg ? 'KHỚP chính xác' : `LỆCH (có ${numNeg} pha âm, cần ${expNeg})`;
      lines.push(`Số pha có công suất đo âm = ${numNeg}, trường hợp này cần ${expNeg === 0 ? 'không có pha nào âm' : expNeg + ' pha âm'} — ${match}`);
    }
  }

  // Cảnh báo tải lệch
  if (isHighlyUnbalanced) {
    lines.push(`LƯU Ý: Dòng điện các pha lệch nhau nhiều → tỷ lệ công suất kém tin cậy khi không có Pa/Pb/Pc`);
  }
