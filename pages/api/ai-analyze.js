...
- Hình 2, 3, 4: Mẫu tham chiếu của Hạng 1, Hạng 2, Hạng 3 (pha nghi ngờ đã được cắt đúng)
- Hình 5 (nếu có): Mẫu ĐẤU DÂY ĐÚNG — BASELINE để so sánh. Dùng để nhận ra điểm khác biệt của Hình 1 so với trạng thái chuẩn
...
export default async function handler(req, res) {
  ...
  // Load ảnh mẫu bình thường — luôn gửi kèm để AI so sánh baseline
  let b64Normal = null;
  try {
    const normalPath = path.join(process.cwd(), 'public', 'ref_normal.png');
    if (fs.existsSync(normalPath)) {
      b64Normal = fs.readFileSync(normalPath).toString('base64');
    }
  } catch(e) { b64Normal = null; }
...
HÌNH ẢNH:
  Hình 1 = Giản đồ vector thực tế (phân tích chính — đây là hình CẦN PHÂN TÍCH)
...
${b64Normal ? `  Hình 5 = MẪU ĐẤU ĐÚNG (đấu dây bình thường, tất cả CT đúng chiều) — dùng làm BASELINE so sánh, giúp nhận ra điểm bất thường trong Hình 1` : ''}
...
4. Viết ngắn gọn đủ 3 hạng, sau đó mới kết luận`;
  ...
  // Gửi: Hình 1 (thực tế) + Hình 2,3,4 (mẫu 3 TH) + Hình 5 (bình thường)
  const content = [
    { type:'image', source:{ type:'base64', media_type:'image/png', data: b64 } },
    ...refImages.map(r=>({ type:'image', source:{ type:'base64', media_type:'image/png', data: r.b64 } })),
    ...(b64Normal ? [{ type:'image', source:{ type:'base64', media_type:'image/png', data: b64Normal } }] : []),
